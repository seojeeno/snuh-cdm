...
wants <- c("DatabaseConnector", "moonBook", "SqlRender", "RPostgreSQL", 
           "lubridate", "RDocumentation", "tidyverse", "lubridate", "readxl", "survival", "survminer", "dplyr", "ggplot2", "ggthemes")
...
data$count[data$gender_source_value == "F"] = -data$count[data$gender_source_value == "F"]
...
brks <- seq(-3000, 3000, 500) #5백 단위로
length(brks)
lbls = paste0(as.character(c(seq(3000, 0, -500), seq(500, 3000, 500))))
length(lbls)

ggplot(data, aes(x = age_group, y = count, fill = sex)) +   # Fill column
  geom_bar(stat = "identity", width = .6) +   # draw the bars
  scale_y_continuous(breaks = brks , labels = lbls) +
  coord_flip() +  # Flip axes
  labs(title="Methimazole Treatment Group Episode Population Spread") +
  theme_tufte() +  # Tufte theme from ggfortify
  theme(plot.title = element_text(hjust = .5), 
        axis.ticks = element_blank()) +   # 제목 중앙으로 
...
mildcohort_data <- data %>% filter(episode %in% mildepisode$episode)
mildcohort_data

extremecohort_data <- data %>% filter(episode %in% extremeepisode$episode)
extremecohort_data 


brks <- seq(-10, 10, 1) #5백 단위로
length(brks)
lbls = paste0(as.character(c(seq(10, 0, -1), seq(1, 10, 1))))
length(lbls)
...
mildepisode
severeepisode
nrow(data %>% subset(data$mildevent == 1))
nrow(data %>% subset(data$severeevent == 1))
...
for (i in data$episode) {
  if (i %in% mildepisode$episode) {
    temp <- measurement_mild %>% subset(measurement_mild$episode == i)
    print(temp)
    (data %>% subset(data$episode == i))$mild_event_date = temp[1,]$measurement_date
  }
}
##################################################################################
...
##############################################################################
(data %>% subset(data$episode == 1))$mild_event_date

nrow(data %>% subset(data$mildevent == 1))

data %>% subset(data$mildevent == 1)
```{r}
data[1,]
```
...
```{r}
data2 <- data2 %>% 
  mutate(event_date = replace(event_date, event_date == '1900-01-01', NA))
```

```{r}
data2 <- data2 %>% mutate(days_before_event = ifelse (is.na(event_date) == 0, (event_date - drug_exposure_start_date), (drug_exposure_end_date - drug_exposure_start_date)))
